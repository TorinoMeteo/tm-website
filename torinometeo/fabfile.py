...
def production():
    ...
    env.user = 'torinometeo'
    env.path = '/home/torinometeo/www/torinometeo'
    env.hosts = ['ws.torinometeo.org']
...
def restart_uwsgi():
    ...
    with settings(warn_only=True):
        ...
        sudo('kill -9 `cat /tmp/project-master_torinometeo.pid`')
        sudo('rm /tmp/project-master_torinometeo.pid /tmp/uwsgi_torinometeo.sock')
...
def dump_db_snapshot():
    ...
    sudo('mysqldump --user torinometeo --password dbtorinometeo > %s' % (remote_tmp_file_path))
