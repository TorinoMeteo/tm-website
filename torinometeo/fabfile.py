...
def dumpDbSnapshot(ctx):
    ...
    c.run(f"mysqldump --no-tablespaces --user torinometeo --password dbtorinometeo > {remote_tmp_file_path}",  # noqa
          pty=True,
...
def restart_uwsgi(ctx):
    ...
    c.run('kill -9 `cat /tmp/project-master_torinometeo.pid`', warn=True)
    c.run('rm /tmp/project-master_torinometeo.pid /tmp/uwsgi_torinometeo.sock', warn=True)  # noqa
